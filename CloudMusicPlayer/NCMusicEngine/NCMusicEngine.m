...
+ (void)clearCache {
    ...
    NSError *error = nil;
    NSArray *subPaths = [fileManager contentsOfDirectoryAtPath:[NCMusicEngine cacheFolder] error:&error];
    
    if (!error && subPaths) {
        
        for (NSString *path in subPaths) {
            
            [fileManager removeItemAtPath:[[NCMusicEngine cacheFolder] stringByAppendingPathComponent:path] error:nil];
        }
    }
